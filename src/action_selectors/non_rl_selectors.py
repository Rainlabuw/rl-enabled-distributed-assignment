...
class AlphaBetaHatSelector(object):
    """
    Greedy action selection used for pretraining, using \alpha(\hat{\beta}) as the
    benefit matrix rather than \mathbf{Q}

    Input: batch to take greedy action on.
...
REGISTRY["alpha_beta_selector"] = AlphaBetaHatSelector

class HAALSelector(object):
    """
    Implementation of HAAL, a optimization-based strategy, for action selection.
    From "Centralized and Distributed Strategies for Handover-Aware Task Allocation in Satellite Constellations", Holder et al.
